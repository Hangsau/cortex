...
> 報告產出時間：2026-06-17
>
> 本研究**不建議**：① **任何「自陳 → 感知／技術品質判斷」的推論**——例如「泳者說 X = 到位」「我覺得手滑過水 → 手的感知不好／動作不好」「換氣嗆水 = 沒抓到水」這類對應。**禁的是這個判斷動作本身，與放不放公開頁無關**：中間那條因果鏈不成立（一句自陳的感覺或一個外顯症狀，未必對應某個特定的感知或技術缺陷）。連帶 A/B/C 型診斷、typical_speech、main_problem 等診斷工具一律不上呈現層。② 任何形式的「進站先選身份」分流。③ 把內容複製成教練版／家長版／選手版多份。
...
- **是使用者自選情境、不是系統下判斷**：卡片只描述使用者自己能對號的處境，點下去終點是 drill／概念／支柱；卡片與終點都**不得**出現「你卡這代表你 X 感知不好／動作不好」這類反推判語（同 §1 硬約束 ①）。
...
### O2 — 痛點卡的命名口吻（在禁區內的有限選擇）

**硬約束（不可拍板、直接排除）**：痛點卡**禁止做任何「自陳 → 感知／技術品質判斷」的推論**。例如「換氣嗆水（可能 L0 呼吸感知未穩）」「划手沒力 = 沒抓到水」「我覺得手滑過水 → 手的感知不好／動作不好」這類對應**通通禁止**——理由是中間那條因果鏈不成立：一個外顯症狀或一句自陳的感覺，未必對應某個特定的感知或技術缺陷（「手滑過水的感覺」未必真的是沒抓到水）。這不是放不放公開頁的問題，是這種判斷**本身無效**。

因此痛點卡**只能是「可觀察的客觀困擾 → 路由到對應 drill 集合」**，不附帶任何歸因或品質評語。可拍板的只剩措辭層：

- **純客觀困擾命名**：「換氣會嗆 / 游一下腳就沉 / 25m 就喘」——只描述使用者自己能確認的外顯現象，點進去是該現象對應的練習，不告訴他「這代表你哪裡不好」。
- **客觀困擾 + 中性導覽**：在卡片內用「想練這個環節可以從這裡開始」這類**方向性**措辭連回 L0–L6 座標系，但**不**反推使用者「現在在哪一級」。

**需委託人決定的只有**：痛點卡要不要在卡內附「方向性導覽」連回 L0–L6（中性、不反推位置），還是純粹只列困擾＋drill 連結。兩種都不碰被禁的推論。

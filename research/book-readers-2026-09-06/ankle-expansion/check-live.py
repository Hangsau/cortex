"""Compare deployed ankle readers with the accepted local build."""
import argparse
...
def main():
    ...
    for chapter in ['kinesiology/ch14', 'basic-biomechanics/ch09']:
        ...
        original = (['foot-map', 'adaptation', 'windlass', 'foot-muscles']
                    if chapter == 'kinesiology/ch14' else
                    ['arch', 'axes', 'tiptoe-model', 'dynamic-foot'])
        check(chapter + ' original four headings', all(a in live.ids for a in original))
        if chapter == 'basic-biomechanics/ch09':
            check('new ankle load diagram', 'rd-ankle-load' in live.classes)
            sliders = [a for a in live.inputs.values() if a.get('type') == 'range']
            check('default ankle slider and disabled no-JS state', len(sliders) == 1 and
                  all(sliders[0].get(k) == v for k, v in {'min': '5', 'max': '15', 'step': '0.5', 'value': '10'}.items()) and 'disabled' in sliders[0])
            check('default ankle force equilibrium', 'd＝10cm；跟腱力A＝1200N；踝反力J＝1800N' in live.prose())
        def cross(links):
            return {urlsplit(urljoin(url, h)).path + '#' + urlsplit(h).fragment for h in links if '/library/' in h and '#' in h}
        check(chapter + ' cross-chapter links match', cross(live.links) == cross(local.links), len(cross(live.links)))
        for target in sorted(cross(live.links)):
            parts = urlsplit(urljoin(BASE, target))
            check('cross-chapter anchor ' + target, parts.fragment in Page(fetch(parts._replace(fragment='').geturl())).ids)

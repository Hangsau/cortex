"""Compare deployed hip readers with the accepted local build."""
import argparse
...
def main():
    ...
    for chapter in ['kinesiology/ch12', 'basic-biomechanics/ch08']:
        ...
        if chapter == 'kinesiology/ch12':
            check('new hip support diagram', 'rd-hip-support' in live.classes)
            sliders = [a for a in live.inputs.values() if a.get('type') == 'range']
            check('default hip slider and disabled no-JS state', len(sliders) == 1 and
                  all(sliders[0].get(k) == v for k, v in {'min': '0', 'max': '120', 'step': '5', 'value': '0'}.items()) and 'disabled' in sliders[0])
            check('default static force calculation', 'C＝0N；M＝1200N；R＝1800N' in live.prose())
            check('original four headings', all(a in live.ids for a in ['hip-geometry', 'pelvis-on-femur', 'hip-balance', 'joint-load']))
        def cross(links):

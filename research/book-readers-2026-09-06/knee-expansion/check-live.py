"""Compare deployed knee readers with the accepted local build."""
import argparse
...
def main():
    ...
    for chapter in ['kinesiology/ch13', 'basic-biomechanics/ch07']:
        ...
        original = (['knee-motion', 'meniscus-ligament', 'patella-leverage', 'contact-stress']
                    if chapter == 'kinesiology/ch13' else
        ...
        if chapter == 'basic-biomechanics/ch07':
            check('new knee compartment diagram', 'rd-knee-compartments' in live.classes)
            sliders = [a for a in live.inputs.values() if a.get('type') == 'range']
            check('default knee slider and disabled no-JS state', len(sliders) == 1 and
                  all(sliders[0].get(k) == v for k, v in {'min': '-40', 'max': '40', 'step': '2', 'value': '0'}.items()) and 'disabled' in sliders[0])
            check('default equal force distribution', 'M＝0N·m；內側FM＝1200N；外側FL＝1200N' in live.prose())
        def cross(links):

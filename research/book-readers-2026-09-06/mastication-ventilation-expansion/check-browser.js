...
const shots = path.resolve(root, '../../tmp/book-translation-mastication-ventilation');
fs.mkdirSync(shots, { recursive: true });
const chapters = ['kinesiology/ch11', 'kinesiology/ch09', 'kinesiology/ch10'];
  ...
  try {
    ...
    for (const chapter of chapters) {
      ...
      for (const href of hrefs) {
        if (new URL(href).hash && (chapter.endsWith('ch11') || href.includes('/kinesiology/ch11/'))) targets.set(href, chapter);
      }
    }
    await page.goto(base + 'library/kinesiology/ch11/');
    check('original four anchors', ['jaw-motion', 'mastication', 'ventilation', 'expiration'].every(a => anchors(chapters[0]).includes(a)));
    ...
    for (const anchor of ['jaw-motion', 'mastication', 'ventilation', 'airflow-pressure', 'intercostal-regions', 'expiration', 'chapter-bridge']) {
    ...
    await page.locator('#expiration').scrollIntoViewIfNeeded();
    await page.screenshot({ path: path.join(shots, 'mobile-expiration.png') });
    check('four-phase table fits', await page.locator('.rd-prose table').evaluate(el => el.getBoundingClientRect().right <= innerWidth));
    ...
    await page.goto(base + 'library/kinesiology/ch11/');
    const figure = page.locator('[data-airflow]'), slider = figure.locator('input');
    check('default flow', (await figure.locator('output').textContent()).includes('Q＝+0.50 L/s（流入）'));
    await slider.press('Home');
    for (let step = 0; step <= 40; step++) {
      if (step) await slider.press('ArrowRight');
      const pressure = (step - 20) / 10, flow = (20 - step) / 20;
      const state = await figure.evaluate(el => ({
        pressure: Number(el.querySelector('input').value), text: el.querySelector('output').textContent,
        desc: el.querySelector('desc').textContent,
        direction: el.querySelector('[data-airflow-direction]').textContent,
        flowText: el.querySelector('[data-airflow-value]').textContent,
        visibility: el.querySelector('[data-airflow-arrow]').getAttribute('visibility'),
        tip: el.querySelector('[data-airflow-tip]').getAttribute('d'),
        line: el.querySelector('[data-airflow-line]').getAttribute('d')
      }));
      const signedFlow = (flow < 0 ? '−' : flow > 0 ? '+' : '') + Math.abs(flow).toFixed(2);
      const arrow = flow > 0 ? state.tip.startsWith('M55 213') && state.line === 'M55 120 V200' :
        flow < 0 ? state.tip.startsWith('M55 120') && state.line === 'M55 213 V133' : state.visibility === 'hidden';
      check('keyboard pressure ' + pressure + ' flow and direction', Math.abs(state.pressure - pressure) < 1e-8 &&
        state.text.includes('Q＝' + signedFlow + ' L/s') && state.flowText.includes(signedFlow) && state.desc.includes(signedFlow) &&
        state.direction.includes(flow > 0 ? '流入' : flow < 0 ? '流向外界' : '無氣流') && arrow &&
        (flow === 0 || state.visibility === 'visible') && !state.text.includes('−0.00'), state);
    }
    ...
    for (const width of [320, 1360]) {
      ...
      for (const theme of ['paper', 'night']) {
        ...
        for (const state of ['in', 'zero', 'out']) {
          await slider.press('Home');
          const increments = state === 'in' ? 10 : state === 'zero' ? 20 : 30;
          ...
          await figure.screenshot({ path: path.join(shots, `airflow-${width}-${theme}-${state}.png`) });
        }
      }
    }
    const ctx = await browser.newContext({ javaScriptEnabled: false, viewport: { width: 320, height: 1000 } });
    const plain = await ctx.newPage();
    await plain.goto(base + 'library/kinesiology/ch11/');
    ...
    check('no-JS default diagram', await plain.locator('[data-airflow] input').isDisabled() &&
      (await plain.locator('[data-airflow] output').textContent()).includes('Q＝+0.50 L/s（流入）'));

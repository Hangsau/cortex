"""Compare deployed mastication and ventilation readers with the accepted local build."""
import argparse
...
def main():
    ...
    for chapter in ['kinesiology/ch11', 'kinesiology/ch09', 'kinesiology/ch10']:
        ...
        if chapter == 'kinesiology/ch11':
            check('new airflow diagram', 'rd-airflow' in live.classes)
            sliders = [a for a in live.inputs.values() if a.get('type') == 'range']
            check('default airflow slider and disabled no-JS state', len(sliders) == 1 and
                  all(sliders[0].get(k) == v for k, v in {'min': '-2', 'max': '2', 'step': '0.1', 'value': '-1'}.items()) and 'disabled' in sliders[0])
            check('default pressure-flow calculation', 'Q＝+0.50L/s（流入）' in live.prose())
            check('original four headings', all(a in live.ids for a in ['jaw-motion', 'mastication', 'ventilation', 'expiration']))
        def cross(links):

/* 週期規劃器（plan 模式同舊 vortex-periodization-planner.js）；
   規則與訓練語言來自 data/periodization/decisions.yaml，
   此處只做輸入驗證、規則匹配與純文字輸出。 */
(function (root, factory) {
  ...
  if (typeof document !== 'undefined') {
    document.querySelectorAll('[data-pz-planner]').forEach(api.mount);
  }
})(typeof globalThis !== 'undefined' ? globalThis : this, function () {
  'use strict';

  function evaluate(guide, raw) {
    if (!guide || guide.schema_version !== 1) {
      throw new Error('資料版本不相容，請使用下方決策表。');
    }
    ...
    for (const key of ['sessions', 'minutes']) {
      ...
      if (!Number.isSafeInteger(input[key]) || input[key] < spec.min || input[key] > spec.max) {
        throw new Error(spec.label + '需介於 ' + spec.min + ' 與 ' + spec.max + ' 之間。');
      }
    }
  ...
  function describe(guide, result) {
    ...
    const sections = [
      { title: '這次的條件', paragraphs: [
        p.fields.map(f => selected[f.key].label).join(' · '),
        '每週最多 ' + input.sessions + ' 次，每次最多 ' + input.minutes + ' 分鐘（含熱身與收操，不要求練滿）。'
      ] },
      { title: '為什麼先做這件事', paragraphs: [
        rule.affirmative_conclusion, selected.population.note, selected.event.note
      ] },
    ...
    sections.push({ title: '這個判斷的適用範圍', paragraphs: [
      rule.works_when, '需要重查的情況：' + rule.fails_when, rule.remaining_boundary
    ] });
  ...
  function toText(guide, result) {
    const sections = describe(guide, result);
    const parts = [result.rule.title];
    for (const section of sections) {
      let block = section.title + '\n' + section.paragraphs.join('\n');
      if (section.slots) {
        block += '\n' + section.slots.map((slot, n) => (n + 1) + '. ' + slot.title + '：' + slot.text).join('\n');
      }
      parts.push(block);
    }
    return parts.join('\n\n');
  ...
  function mount(host) {
    const status = host.querySelector('[data-pz-status]');
    let guide;
    try {
      guide = JSON.parse(host.querySelector('[data-pz-config]').textContent);
      if (guide.schema_version !== 1) throw new Error('version');
    } catch (_) {
      status.textContent = '互動資料暫時無法載入，請使用下方完整決策表。';
      return;
    }
    const form = host.querySelector('form.pz-planner-form');
    const output = host.querySelector('[data-pz-result]');
    const title = host.querySelector('[data-pz-result-title]');
    const body = host.querySelector('[data-pz-result-body]');
    const fallback = host.querySelector('[data-pz-copy-fallback]');
    let current = null;
    ...
    function run() {
      ...
      try {
        ...
        for (const section of describe(guide, current)) {
          ...
          if (section.slots) {
            const list = append(body, 'ol', '', 'pz-planner-slots');
    ...
    host.querySelectorAll('[data-pz-example]').forEach(button => {
      button.hidden = false;
      button.addEventListener('click', () => {
        const example = guide.planner.examples.find(item => item.key === button.dataset.pzExample);
        if (!example) return;
    ...
    host.querySelector('[data-pz-copy]').addEventListener('click', async () => {
      if (!current) return;

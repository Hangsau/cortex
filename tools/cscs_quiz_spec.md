...
**同一條的第二種做壞法：干擾項在自己的文字裡承認自己是錯的。** ch05 與 ch06 各出現一批：
`Direction reverses, reads the analysis as a 5 to 10 percent rise`、
`Magnitude inflated to a full 20 to 25 percent reduction`、
「FOR → 急性疲勞 → NFOR → OTS，**順序顛倒**、恢復時間錯置」、
「荷爾蒙種類會完全改變，出現**書中沒有的**新胜肽」。
`reverses`／`inflated`／`顛倒`／`反轉`／`錯置`／`書中沒有` 是**錯因的語言**，寫進選項等於
把答案標出來。**選項只陳述一個說法，評價它是 `why_wrong` 的工作**，驗收會擋（G20）。
修法是把評語換成中性描述（「恢復期最短者需數週」），**不是把尾巴刪掉**——刪光會讓三個選項
的字集趨同而撞 G9。

同類還有**用絕對詞把干擾項寫死**：「Type I 在**任何**刺激下都無法合成肌原纖維」
「次大心率**完全**不變」「心血管**完全**與壓力荷爾蒙無關」。「任何」「永遠」「完全」「絕不」
在生理學裡幾乎必假，考生不必懂也能刷掉。這條沒做成閘（正當用法太多），但一樣是廢題。
...
**限定語一律留在題幹，選項只留「數值 + 最短單位」**——照樣題的 `120 g` / `480 g` / `960 g`。
「量同、單位同、只有數值不同」的另一面，是三個選項的文字會高度重疊：
`每公斤體重 8 至 10 公克` 與 `每公斤體重 3 至 5 公克` 的字元重疊率 0.615，直接撞 G9。
把「每日每公斤體重」這種共同框架移進題幹（「…每日每公斤體重應攝取多少公克？」），
選項就只剩 `8–10`／`3–5`／`12–14`，重疊率立刻掉到門檻以下，而且更貼近官方樣題。
百分比同理：題幹問「濃度為何（%）」，選項寫 `高於 8`／`6 至 8`／`2 至 4`，
不要三個都拖著「介於百分之…的濃度」。**選項共用的字愈多 G9 愈容易撞，而共用的部分本來就屬於題幹。**
...
依官方 domain 權重分配，全 24 章共 **967 題**（約五份完整考卷的量）。
每章的認知層級配比直接複製該 domain 的官方配題（`_dco.yaml` 的 `cognitive`）。

| 章 | domain | 題數 | recall | application | analysis | 其中英文題 |
|---|---|---|---|---|---|---|
| ch01–ch07（各） | sf1 運動科學 | 35 | 10 | 20 | 5 | 13 |
| ch08 | sf2 運動心理 | 100 | 30 | 60 | 10 | 33 |
| ch09–ch11（各） | sf3 營養 | 20 | 5 | 10 | 5 | 8 |
| ch12–ch13（各） | pa3 課表執行 | 55 | 8 | 30 | 17 | 18 |
| ch14–ch16（各） | pa2 動作技術 | 48 | 8 | 25 | 15 | 15 |
| ch17–ch22（各） | pa1 課程設計 | 38 | 3 | 18 | 17 | 13 |
| ch23–ch24（各） | pa4 組織行政 | 40 | 28 | 12 | 0 | 13 |

**這張表在 2026-09-14 從 385 題上調到 967 題，比例不動、只等比放大 2.5 倍。**
上調的理由是派工的成本結構：一次發包的固定成本（讀完該章 60–96 條 item、吃下 6500 字的
規格 prompt、跑完六輪自驗迴圈）與它寫 14 題或 35 題幾乎無關，所以小批次等於把固定成本
攤在極少的產出上。人工逐題查證的成本則是按題數線性成長、與發包次數無關——換句話說
把批次做大不會讓驗收變貴。967 題吃掉 item 容量（1557 條 × 每條上限 2 題 = 3114）的 31%，
留下的餘裕足夠日後補題。**ch01–ch03 是舊配額（各 14 題）下產出並已人工驗過的，
最後再跑一輪追加題補到 35，既有題目不重寫。**

**測驗組合題（R6）的下限**：ch12–ch13 每章至少 15 題、ch17–ch22 每章至少 10 題必須是運動員側寫題。
...
| G3 | 三個選項長度變異係數 ≤ 0.4 |
| G4 | 最長選項 ≤ 最短選項 × 1.5 |
| G5 | 每個干擾項有 `why_wrong`，與選項本文重疊率：英文比**詞** ≤ 0.45、中文比**字元** ≤ 0.6 |
| G6 | 題幹是完整問句（中文 ≥ 12 字收「？」；英文 ≥ 8 詞收「?」），且扣掉來源 `q` 之後仍自帶內容 |
| G7 | `item` 存在於來源章；`locator` 與來源逐字相同；`meta` 對應檔名 |
| G8 | 同章同 item 的題幹不得重複 |
| G9 | 三個選項兩兩重疊率：英文比**詞** ≤ 0.5、中文比**字元** ≤ 0.6 |
...
| G16 | 選項本文不得含分號（R1） |
| G17 | 希臘字母只能當術語的一部分（`α-motor`、`µm` 可以；`ν`、`ΔP` 不行）（R1） |
| G18 | 同一題的選項不得混用度量單位（`nm` 配 `µm` 即失敗）（R5） |
| G19 | `why_wrong` 不得是電報體（英文 < 8 詞、中文 < 12 字即失敗）（R3） |
| G20 | 選項本文不得自承錯誤（`reverses`／`inflated`／`顛倒`／`反轉`／`錯置`／`書中沒有`…）（R3） |
...
2026-09-14 的 ch01 試作證實：**寫在散文裡而沒有對應閘門的規則，發包出去一律不會生效**。
第一輪補了 G15／G16 重跑，被擋的兩項當場修好，同一份 prompt 裡純散文的四項一項都沒改。
所以每章走兩輪，中間不省：

```
python tools/cscs_quiz_make_prompt.py chNN            # 第一輪：出題
python tools/cscs_quiz_make_prompt.py chNN --review   # 第二輪：抓爛干擾項
```

第二輪的任務範圍刻意很窄——題庫已經過閘，只准動 `options` 的 `text` 與 `why_wrong`，
專抓四類機器驗不到的東西：不讀書也知道是假的干擾項、自己拼出來的術語、
三個選項不在同一層次、正解只是把題幹換句話說。窄任務比「再檢查一次」有效，理由同上：
模型的注意力預算有限，一輪要同時顧 14 題的格式與內容時，先掉的一定是內容。

**新發現的缺陷樣態一律寫成具名例子塞進 `cscs_quiz_review_prompt.md`，不要寫成通則。**
ch01 的觀察是散文規則會被忽略，但**審查者對 prompt 裡點名的每一個具體例子都動手修了**。
ch02 補進去的兩個例子就是這樣來的：正解 `tendon insertion` 只是抄題幹主詞、
`isometric` / `lifting up` / `lowering down` 裡只有正解是術語——兩者都全綠過閘，
靠的是人工逐題看出來的。

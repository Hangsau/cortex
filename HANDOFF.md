...
**下一步建議**：① **ch02–24 的深度層補完（1515 條）是唯一剩下的大工程**——`detail`/`terms`/`numbers` 空著，深度層整塊不渲染，畫面不會有半成品但也沒有 ch01 的密度；派工時每章一單位、補完立刻 `cscs_check` + commit，跨多個配額視窗接力；② 遮答自測仍是全頁 toggle，「逐條遮／已答對不再遮」需要 localStorage 層，未做；③ `layouts/library/book.html` 與書庫其他 layout（library.css / bookshelf.css）調性未對齊。

...
## 首頁漏接讀本入口與第 1 章加深（2026-09-06）

使用者回報線上看不出更新。正式首頁 HTTP 200，但沒有兩冊讀本網址，只有頁尾書庫；前次「已部署」僅驗深層頁，漏了首頁可發現性。已將兩册讀本設為「肌力與動作科學」常駐主要入口，第一個捷徑改為「讀肌動學與生物力學」；既有 CSCS／概念入口仍在。首頁資料擴為 19 個目的地，30/30 瀏覽器檢查通過，320 px 頁面 2.98 個 viewport。

使用者明確准許較便宜模型初譯；兩個 `gpt-5.6-luna` 子代理完成第 1 章分段初譯，正在由主代理核對與整合，尚未把初稿當成正式完整翻譯。工作 checkpoint：`research/book-readers-2026-09-06/ch01-expansion/RUN.md`。

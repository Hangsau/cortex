...
**下一步建議**：① **`related` 補底輪**（唯一還開著的內容債）——ch04／ch05／ch06／ch07／ch08／ch19／ch23 這七章跑一輪，直接複用 `.prompts/ch17-related.md` 這份窄範本，改章號即可（只動 `related`、其餘欄位逐位元凍結，已驗證）。ch24 已定稿，ch23 可以開跑了。跨章下限照模板寫的「全章至少一半跨章」，並沿用 ch22 起驗證有效的**具體鄰居章號清單**寫法（只喊「要跨章」沒用，ch21 只連到 5 章、給了清單的 ch22 連到 15 章）。串行派發，一章驗收 + commit 後才派下一章。② **欠 ch01–ch03 一次卡片補課**：這三章的卡片只做過點狀修補（`33d7df5` 修了 ch02 三張），從沒回源書整批重寫；ch13/ch15/ch21/ch23/ch24 已在對帳時重寫過，可跳過。③ 遮答自測仍是全頁 toggle，「逐條遮／已答對不再遮」需要 localStorage 層，未做。④ `layouts/library/book.html` 與書庫其他 layout（library.css / bookshelf.css）調性未對齊。⑤（配額）5H >80% 一律不開新批次，排 shotclock 等下一窗。

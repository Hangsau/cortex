...
- **2026-09-26 P2–P4 完成（預覽站 `/cortex/next/`）**：書房全部系列（兩冊讀本 33 章＋跨書主題對照＋學習地圖、CSCS 24 章＋概念索引＋選擇題 1,033 題＋閃卡 1,252 張、大腦喜歡這樣學 20 技法、UST 10 章＋18 策略、氣質 5 篇＋9 維度＋自測）、Vortex 剩餘內容（水感導讀、呼吸、週期化＋規劃器、ADM 三頁＋背景、心理長讀、骨關節、動作圖譜、級別頁 L 指標、23 篇技術深探／感知橋接／水感框架長文）、全站首頁＝書房。工作單 L1/L3–L5、V2–V7、T1–T4 由 M3 實作；視覺層、共用版型、驗收全由 Claude。**20 張驗收全綠；P4 全站連結稽核 957 頁／40,126 條站內連結零斷連結零錯錨點；舊站 121 個網址全數對應（111 同路徑＋10 轉址頁 `next/static/`）**
- **P5 正式切換完成（2026-09-26，使用者授權「原本的封存或直接拿掉都可以」）**：舊版先打 git 標籤 `legacy-site-2026-09-26` 並推上 GitHub 封存；移除 `layouts/`、`static/`、`data/home.yaml`、舊 Playwright 稽核 6 支、未掛載的舊內容空殼；`hugo.next.toml` 改為 `hugo.toml`（baseURL `/cortex/`、publishDir `public`）；shortcode 移到 `next/layouts/_shortcodes/`；CSCS 中文章名改存 `next/data/cscs_titles.yaml`；`deploy.yml` 改跑資料產生器後建新版；`check.py` 與 `build_redirects.py` 改對正式網址（V2 從封存標籤讀舊水感導讀原文比對）；CLAUDE.md、MAP.md 依新架構重寫（舊版面規則全移除）。22 項驗收全綠、舊網址 121 筆全對應。**`/cortex/next/` 預覽網址已不存在**，預覽期在那裡累積的閱讀進度不會帶到正式網址
- 驗收：`python -X utf8 next/specs/check.py <W>`（W1、W4–W8、L1、L3–L5、V2–V7、T1–T4、P4）；轉址 `python -X utf8 tools/build_redirects.py`（舊網址清單 `next/legacy_urls.txt`，舊站刪掉後無法重建，勿刪）
- **本輪修掉的 M3 缺陷**（驗收抓不到、Claude 抽查才抓到）：CSCS 數字 `%!s(uint64)` 格式錯誤與概念連結相對路徑、週期化雙／三週期因 key 比對錯而整段消失＋柔軟度欄位名錯、選擇題／閃卡漏載共用樣式、8 個 M3 產出檔自稱「Claude 手寫」、呼吸章 320px 溢出。另 Claude 自己的錯：把 `legacy_urls.txt` 放進 `next/data/` 導致 Hugo 建置失敗、讓 M3 白跑 81 回合
- **2026-09-26 跨系列知識連結（完成）**：候選 `tools/crosslink_candidates.py`（3,367 對）→ 初審 `tools/crosslink_judge.py`（2,300 判相關）→ 低分段（score<0.08）嚴格複審 `--verify-below 0.08`（1,502 對留 1,017；抽查初審低分段約 20% 牽強才加這關，複審偏嚴、寧可少連）→ 組裝 `tools/build_crosslinks.py` 產 `next/data/crosslinks.json`（**採用 1,815 對、1,110 個單位**：肌動學 463 節、生物力學 330 節、CSCS 206 條、Vortex 111；理由文字的 A／B 依方向改成「這裡／它」）。呈現：`next/layouts/_partials/library/xl-templates.html` 只輸出本頁用得到的 `<template data-xl-for>`，由 `next/assets/js/study.js` 插到讀本小節標題下、CSCS 條目末尾、Vortex 專頁段落；Vortex 知識點頁在「接著看」加「教科書怎麼說」。P4 稽核涵蓋這些連結
- **2026-09-26 閱讀陪伴（給拖延者，Vortex 以外為主）**：`study.js`＋`shell.css` 末段。每節標「約 N 分鐘」、讀完自動打勾（小節結尾捲過畫面上方 30% 且停留 ≥ 分鐘數 1/4）、章首進度列＋「從還沒讀的那一節接著讀」、讀完一節的輕提示、系列首頁每章／全書進度條、書房「接著讀」＋「今天只讀一節」、無紀錄時「不知道從哪開始？挑一個短的」、MNFL／UST／氣質首頁「今天試這一個」（每日換）。刻意不做連續天數與催促。資料只在 localStorage `cortex-study-v1`；切換正式網址後預覽期的進度不會帶過去
- **2026-09-26 使用者回報「Vortex 內容變少」→ 查為導航漏接**：呼吸、心理層、骨關節、動作圖譜、ADM、週期化、什麼是水感七頁早已建好卻不在左側目錄與首頁。已補（`vortex/rail.html` 新增「主題」「長期規劃」分組；`vortex/home.html` 加「主題與長期規劃」）；P4 稽核加孤兒頁檢查（`/library/` 為舊網址書房落點，豁免）。比對舊站另發現找練習篩選從 12 軸退成 3 軸，已恢復（標籤在 `next/data/drill_axes.yaml`，`filter.js` 改通用多軸）。**仍缺：舊「查資料」跨泳式搜尋**（舊 `/vortex/database/` 現轉址 Vortex 首頁），使用者先前說搜尋不好用，未主動補，待使用者決定
- **2026-09-26 書房「今天的短篇」改為每次隨機**（使用者決定：本機閱讀紀錄不可靠，不以讀過沒讀過篩、也不固定每日一組）：`tools/build_starters.py`（需先跑 `build_library.py`）產 `next/data/starters.json`（讀本每章第 1、4、7…節 381、CSCS 每章一筆「隨機五題」24、學習技法＋課堂策略 38、氣質 14，72 KB），`study.js` 每組 Math.random 挑一個、「換一組」重抽。**CSCS 隨機五題**：連到 `quiz/?ch=chNN&n=5`；`cscs-quiz.js` 讀 `ch`（可為 `random`）與 `n` 參數，直接選章、打亂取前 n 題開始。P4 稽核比對前會去掉網址參數
- **2026-09-26 課表與間歇計時（`/vortex/workout/`，Claude 手寫）**：填 50/100/200/400 PB → CSS（有 200＋400 用兩點法；只有 400 用 +4.5 s/100 捷徑；只有 200 用 Riegel；只有 50＋100 依使用者指示外推並標低確定度、附規則表「單一成績禁止」原文；單一 50 或 100 最粗）→ 技術／耐力／速度 × 50/100/200 每趟目標秒數與出發間隔；衝刺換算＝PB ÷ 強度%（70–100 拉桿＋80–100% 速查），休息／趟數依 Sp 區；課表可編輯；全螢幕計時器：準備 10 秒倒數嗶、出發長嗶、到目標秒數提示音、「到牆」記實際秒數（沒按就依設定自動進休息）、休息最後 3 秒嗶、螢幕常亮。處方數字全讀 `next/data/swim_rules.json`，由 `tools/sync_swim_rules.py` 從 swim-coach `rules/menu_rules.yaml` 同步（規則表改了要重跑並 commit；CI 讀不到 swim-coach）。輸入與課表只存 localStorage `cortex-swim-v1`。Playwright 假時鐘跑完整流程（準備→游→到牆→休息倒數→第 2 趟→自動到目標→完成）通過。**待議**：50＋100 外推與規則表的「禁止」衝突已寫進 swim-coach HANDOFF 給使用者決定是否改規則；完整課表產生器（暖身／主課／緩和）尚未移植
- **2026-09-26 課表工具改版（使用者要求：四式＋混合式、整份課表再轉 HIIT、drill 可挑內建或自訂、每列選器材）**：成績表改為各式所有比賽項目（自由 50–1500、仰蛙蝶 50–200、混合 100–400、短／長池），每式分開算 CSS（多筆非 200＋400 時用個人疲勞指數外推；混合式無成績取各式平均，都標低確定性）；課表改成多段（暖身／Drill／主課／緩和，可增刪改名排序）× 多列（趟數×距離、泳式含自選、游／腳／手／drill、強度 不設目標／技術／耐力／速度／衝刺%／自訂秒數、休息、器材、備註），腳與 drill 不用游泳成績算目標；drill 從 Vortex 179 個內建挑（依泳式分組，附動作提示與連結）或⑥自訂（名稱、泳式、提示、預設器材），另可自訂器材；可存多份課表、總覽可複製成文字；計時器照整份課表逐列逐趟跑，無目標的趟靠「到牆」，顯示 drill 提示與器材、休息時預告下一列。器材中文名在 `next/data/swim_equipment.yaml`（drill 資料出現新器材 key 要補）。localStorage 改 `cortex-swim-v2`，舊 v1 自動搬移。Playwright 全流程（四式成績、自訂 drill／器材、三段課表、計時跑完、重新載入保留、390／320 無溢出）通過。**已知**：Vortex drill 頁的「器材」仍印英文 key（`unit.html`），未改
- **2026-09-27 課表工具第三輪（使用者手機實測回報）**：① 手機數字鍵盤打不出冒號 → 所有時間改「分｜秒」兩格（numeric／decimal 鍵盤，失焦自動整理成分:秒，秒格打 90 會變 1｜30）；② 「主課無法增加」根因：輸入框失焦時整個課表重畫，把緊接的按鈕點擊吃掉 → 編輯器改為打字只更新狀態與衍生文字、只有按鈕／下拉才重畫（`workout.js` 檔頭鐵則）；「＋ 在『段名』加一列」改成整寬按鈕；③ 每列加「幾組」與「組間休息」（例 2 組 × 8 × 25 m）；④ ⑤ 改成編號清單＋列印（只印清單，JS 複製到 body 層 `#wk-print`）＋從清單進入 HIIT；⑤ 計時中臨場調整：目標 ±1 秒、休息 ±10 秒，套用到當前這一列剩下的趟（含組間休息），只影響這次計時，結束畫面列出調整紀錄。iPhone 13 模擬全流程測過、390 無溢出
- **2026-09-27 課表工具第四版：公式改走 Vortex canonical＋週期推算**：使用者抓到「蝶式 50 m 38 秒推 25 m 95% 竟比自由式 50 m 34 秒推的還快」——根因是跨距離外推（借跑步 Riegel 1.06、再把個人指數夾限在 1.02–1.25，蝶式被夾到 1.25）。修法：所有配速公式改讀 `data/periodization/set-design.yaml` 的 generator（CSS 四級：200＋400 實測／只有 400 ÷0.92 估計／兩筆 ≥50 m 用 log-log 擬合個人指數推 400 外推／其餘自校準不給秒數；有氧 ×1.06、×1.09；速耐 T100×1.10 🟠；賽速＝賽事 PB ÷ 段數）；衝刺 % 只用同式同距離 PB，沒有就不換算（只附「長距離比賽配速切段」參考）；拿掉混合式四式平均、單點外推。成績欄加 25 m、顯示照輸入不補 .0；新列秒數不自動帶，選強度後顯示建議休息＋「套用」鍵；加入主課改用提示訊息不跳頁。新增 ③ 週期與本週建議：賽事日期／今天／可選週期開始日／比賽泳式距離／每週次數 → 推算階段（大週期預設 15 週 🟢 Hellard；各期比例取慣例中值 🟠；減量 8–14 天 🟢）、時間軸、純速／賽速／速耐／有氧主次（轉寫 phase_allocation，原文並列）、項目能量來源、減量要點、一週 N 堂配置（weekly_assembly）、3 組推薦主課（set_types 區間中值，一鍵加入）。**swim-coach 規則表現在只供 drill／緩和預設休息**。不給「每週各強度幾成」（canonical 已撤回）
- **2026-09-27 第四版更正（上一版誤解使用者）**：「不要自動帶秒數」原意是**重新整理後不要帶上次的成績**——成績（PB）改為不存進 localStorage（存檔時剔除 pb，並清掉舊存檔裡的），每次開頁空白；課表、自訂 drill、器材照存。休息秒數恢復自動帶：依強度／游法／距離帶建議區間中值（`restAuto`），使用者手改後不再覆蓋，提示列有「改回」鍵；④ 衝刺換算的趟數休息直接帶建議值。週期推算拿掉「比賽泳式／距離」（使用者：比賽可能多項，此功能暫不重要）：推薦組的泳式跟 ② 選中那一式，賽速組給 100 m 樣板、項目在課表列裡選
- **2026-09-27 沒有 25 m 成績也能出衝刺課表**：`baseFor()`——同式同距離 PB 優先；沒有就用同一式最近的較長距離成績平均切段（canonical race_pace_per_rep 公式，例 50 m 34 → 25 m 基準 17.00），標明「偏保守」；不從短距離往上推、不用指數外推。④ 衝刺換算、課表列的衝刺％、推薦純速組都走它
- **2026-09-27 時間欄改單一格＋50→25 教練換算**：時間欄不再分分／秒兩格，改單一格一般鍵盤，可填「1:30」或「90」，照輸入顯示；沒有 25 m 成績時用 `next/data/swim_25_from_50.yaml`（Hang 提供的教練觀測點：自由 34.2→17.7、蝶 37.5→18.7、蛙 53→29.5、48→28，🟠）各式 t25/t50 平均比換算成 25 m 蹬牆衝刺；仰式無資料退回 50÷2。**待補**：仰式觀測點、各式更多點
- **2026-09-27 50→25 改回直接 ÷ 2**（使用者判斷：跳水與蹬牆的差、距離短速度較快兩者抵銷，不必按泳式修正）；`swim_25_from_50.yaml` 已刪除
...
**ch17 完成 38 題並已逐題查證**（22 道閘 0 錯，3 recall / 18 application / 17 analysis，
13 EN / 25 ZH，精準命中配題表；8 個 topic 全數覆蓋、單一 topic 最多 5 題、
38 題落在 38 個 item；模型在 `--fix-only` 也收斂不動，4 題手改，人工查證再抓 4 條，見下節）。
**ch18 完成 38 題並已逐題查證**（22 道閘 0 錯，3 recall / 18 application / 17 analysis，
13 EN / 25 ZH，精準命中配題表；8 個 topic 全數覆蓋、單一 topic 最多 6 題、
38 題落在 38 個 item；`--fix-only` 4 輪 18.7 秒全綠、零手改，但人工查證抓到 3 題重寫，
其中一題正解與題幹方向相反，見下節）。
**ch19 完成 38 題並已逐題查證**（22 道閘 0 錯，3 recall / 18 application / 17 analysis，
13 EN / 25 ZH，精準命中配題表；8 個 topic 拿到 5/5/5/5/5/5/4/4、38 題落在 38 個 item；
第一次出題主題嚴重偏斜，改的是生成器不是題庫，見下節）。
**ch20 完成 38 題並已逐題查證**（22 道閘 0 錯，3 recall / 18 application / 17 analysis，
13 EN / 25 ZH，精準命中配題表；8 個 topic 拿到 5/5/5/5/5/5/4/4、38 題落在 38 個 item；
生成器修好後主題一次就攤平，但挑選器交出 41 題超額，多的 3 題正好是兩組真重複，見下節）。
**ch21 完成 38 題並已逐題查證**（22 道閘 0 錯，3 recall / 18 application / 17 analysis，
13 EN / 25 ZH，精準命中配題表；8 個 topic 拿到 5/5/5/5/5/5/4/4、無超額；
跨題旗標由 18 降到 1，人工查證改寫 10 題，其中一題正解違反來源自己的 D 註解，見下節）。
**ch22 完成 38 題並已逐題查證**（22 道閘 0 錯，3 recall / 18 application / 17 analysis，
13 EN / 25 ZH，精準命中配題表；8 個 topic 拿到 5/5/5/5/5/5/4/4、無超額；
跨題旗標 8 條全清，人工查證改寫 12 題——殘錯破 10 條的原因是本章正解多為
3–6 字中文短名詞，G2 與 G4 幾乎無法同時滿足，見下節）。
**ch23 完成 40 題並已逐題查證**（22 道閘 0 錯，28 recall / 12 application / 0 analysis，
13 EN / 27 ZH，配額格四格全中；8 個 topic 只拿到 7/7/5/5/4/4/4/4——
沒有閘擋 topic 偏斜；題幹撞擊 9 條全清，人工查證改寫 24 處，
最大類是捏造指涉的 `why_wrong`，見下節）。
**ch24 完成 40 題並已逐題查證**（22 道閘 0 錯，28 recall / 12 application / 0 analysis，
13 EN / 27 ZH，配額格四格全中；8 個 topic 各 5 題、40 個 item 零重複——
**第一次不靠人工介入就攤平**，是 topic 區間收成 ±1 的直接效果；
跨題掃描各類別皆 0，人工查證改寫 19 處，最嚴重的是正解與來源相反，見下節）。
**24 章全部出完**。**ch01–ch03 的追加題也已完成**：ch01（`75b662a`）、ch02（`1ff9bf1`）、
ch03（`a5c2002`）各補 21 題到 35 題，原有 14 題逐題比對確認未被更動；三章都是
10 recall / 20 application / 5 analysis、13 EN / 22 ZH，閘 0 錯，跨題掃描殘留旗標
...
**補題已改走 `cscs_quiz_direct.py --topup`，舊的 `cscs_quiz_topup_prompt.md` +
`cscs_quiz_make_prompt.py --topup` 外部 agent 路徑停用**（同全章出題一樣的理由：瓶頸在
agent harness）。差額由 `topup_plan(chid, base_questions)` 算：**先把整章的配額格攤開，
再扣掉既有題庫佔掉的格子**，不能走 `--part` 的最大餘數分配——那條路徑會把 21 題的
零頭湊錯格。ch01–ch03 的差額都是 (21, 6, 12, 3, 8)。新題的可用 item 限定在本章從沒
出過題的那批，所以與舊題撞 item 在結構上不可能。**不要與另一個正在寫 `_quiz_bank/`
的任務同時跑。**
...
### ch17：模型在整章 `--fix-only` 也收斂不動；人工查證抓到一題題幹與正解自相矛盾（2026-09-21）

**出題端：第一次遇到 `--fix-only` 自己也停住。** ch16 的殘留 2 輪就清掉，ch17 的三批分別卡在
5 / 4 錯，整章 `--fix-only ch17` 再跑仍剩 6 錯且不再下降。6 條全是選項本來就是短數字或短術語
的機械問題（G2／G4／G9），模型改不動不是因為不懂題目，是因為字面長度沒有空間。
處置一律是把三個選項的字面長度拉齊、換掉共用骨架，不動考點，4 題手改比再燒幾輪快。
**判斷線：`--fix-only` 跑兩輪錯誤數不降就停，改手改。**

**人工查證抓到 4 條，跨題掃描與 22 道閘都看不到：**

| 題 | 缺陷 | 處置 |
|---|---|---|
| `exercise-order.i03.q1` | **最嚴重**：題幹問「二頭彎舉接錘式彎舉為何該連著排」，但原文正是拿「連續多個拉動作使肱二頭肌疲勞」當**反例**；正解「交替相反肌群」與題幹描述的情境自相矛盾 | 題幹＋三選項全換成推／拉交替的正例（臥推接坐姿划船） |
| `exercise-order.i08.q1` | 兩個誘答寫成**對題目本身的評論**（「本題兩個動作都屬下肢，並非上下肢交替題型」），不是可選的答案 | 換成兩個真正的錯誤因果（先做腿彎舉可提高深蹲負荷／可提高總訓練量負荷） |
| `training-volume.i03.q1` | 正解「肌耐力通常 2 至 3 組」**不在所引 item 的來源內**——該 item 只講肌力的核心 2–6 組與輔助 1–3 組；誘答的錯因還引了別條 item 的內容 | 改考來源真的有的輔助動作組數（1 至 3 組） |
| `exercise-selection.i06.q1` | 正解替腿後肌動作加上原文沒有的「輔助動作」分類，還讓誘答的對比軸變成核心／輔助而非弱側／強側 | 拿掉分類詞，鑑別點單純落在腿後肌 vs 股四頭肌 |

**前兩條是同一種病**：模型把「本章的某條原則」和「題幹描述的情境」黏在一起，
黏錯了仍然讀得通，因為每個句子單獨看都是對的。22 道閘只驗形式，跨題掃描只驗重複，
**只有把題目和它的來源 item 並排讀才抓得到**。`ch17_verify.txt` 那份
「題幹 + 選項 + 錯因 + 來源 Q/A/detail」的並排格式是唯一有效的工具，每章都要出。

**第三條是來源可追溯的問題**：答案本身正確（教材別處確實這樣寫），但不在該題 `locator`
指向的 item 裡，考生照著查會查不到。**查證時要問的不是「這句對不對」而是「這句在不在這條來源裡」。**

改題幹一樣要跑一次跨題掃描（ch16 的教訓）；本次兩個新題幹都沒有撞到 0.55。

**遺留的組成問題（不阻擋）**：5 對題幹落在 0.553–0.607，是「一名……最應……？」的骨架重複；
正解相似度 0.455–0.667 集中在極短字串（「核心動作」4 字、「8RM」3 字、「上肢訓練」4 字）
被掃描的 `min()` 分母放大，屬假訊號。

### ch18：`--fix-only` 一次全綠反而是警訊；三題正解站不住（2026-09-21）

**出題端首次零手改**：三批分別剩 1 / 4 / 0 錯，整章 `--fix-only ch18` **4 輪 18.7 秒全綠**，
和 ch17 卡在 6 錯不動完全相反。它同時把超額的 40 題修到 38 題（丟掉
`mechanical-models.i07.q1` 與 `program-design.i05.q1`）。
**但閘全綠與內容正確無關**——本章人工查證抓到的 3 條，是 ch16 / ch17 以來最嚴重的一批。

| 題 | 病 |
|---|---|
| `equipment-facility.i02.q1` | 問「最應避免的落地面」，正解卻標在「鋪於混凝土的橡膠墊」，而干擾項「未鋪墊的混凝土地面」的錯因自己寫著「混凝土教材明指不建議」——**正解比干擾項更好**。且來源 i02 講的是 6 英吋厚墊與彈跳床，整題與 locator 無關。改成問 20 公分厚墊延長接觸時間 |
| `equipment-facility.i03.q1` | 問 3 公尺淨高「最不適合哪一項」，正解標「深跳」；但來源把立定跳、箱上動作、深跳列為**同一個** 3–4 公尺需求，兩個干擾項的錯因還各自承認「所需高度與箱上跳相同」——**題目在來源裡無解**。改成問 12 公尺短場地限制哪一類，打在來源真正有的 30 公尺直線需求上 |
| `ssc.i02.q1` | 正解「儲能以熱散失，向心期失去反射增強」不在 `ssc.i02` 裡（那是 `mechanical-models.i05` 的內容），跨題掃描也因此把兩者判成 0.800。改成問攤銷期在脊髓層的突觸事件，那才是這條來源寫的 |
| `program-design.i06.q1` | 題幹問「課程長度**與**可見成效最早時程，分別如何」，三個選項只答課程長度，另一半無從作答。題幹收斂成只問週數 |

**這章推翻了「閘全綠 ≒ 這批比較乾淨」的直覺**。ch17 是模型自己修不動、手改 4 題，
查證再抓 4 條；ch18 是模型自己修到全綠，查證卻抓到**兩題根本無解**。
兩章合起來的結論是：`--fix-only` 收斂快慢只反映形式錯誤好不好修，
**對「正解站不站得住」零資訊，不能拿來決定要不要細查**。

**前兩條共用一個新樣態**：干擾項的 `why_wrong` 自己供出了反證
（「混凝土教材明指不建議」「所需高度與箱上跳相同」）。
**查證時先把三個 `why_wrong` 當成一組互相矛盾的證詞讀**，能在比對來源之前就先篩出可疑題。

**跨題掃描在重寫後自己乾淨了**：原本兩對 1.000（都撞上只有兩個字的正解「深跳」）、
一對 0.800、一對 0.556 全數消失，剩下的 5 條都是短數字或「深跳」二字被 `min()` 分母放大的假訊號。
**掃描分數高不必然是重複，但它指的那幾題值得優先細讀**——本章 3 條缺陷有 2 條就在掃描名單上。

**手改腳本的兩個新坑**：① G9 有 `differ_only_in_numbers` 豁免（只差數字的選項不比對），
自寫斷言照抄 0.6 門檻會誤殺既有題（`program-design.i06.q1` 的 6–10 週 / 12–16 週算 0.818）；
② 只改題幹時不要連正解也拿去跑跨題斷言，那些分數是動手前就存在的，會擋住無關的修正。

### ch19：主題偏斜是閘看不到的缺陷；修的是生成器不是題庫（2026-09-22）

**第一次出題 38 題全綠、配題表精準命中，主題卻塌在少數概念上**：
`core-concepts 7 / agility-cod 7 / neuromuscular-basis 7 / program-design-monitoring 7 /
rfd-impulse 6 / training-methods 2 / sprint-speed-components 1 / sprint-technique 1`——
一個叫「速度與敏捷訓練計畫設計」的章，衝刺技術與速度組成各只有 1 題。

**22 道閘與配題表都看不到主題分布**（它們只驗 cognitive／lang 邊際與逐題形式），
唯一看得到的是 `cscs_quiz_cross_scan.py`，而且是間接看到——它報的那對 1.000 重複正解
（`rfd-impulse.i01.q1` vs `core-concepts.i06.q1`，兩題都在問「力量發展速率」）
正是題目擠在同一概念的症狀。**每章跑完掃描時順手數一次 topic 分布，這是目前唯一的偵測管道。**

**修 prompt 不修題庫**：`select_questions` 雖然把 topic 當第二順位排序鍵，
但它只能在模型交出的候選池裡挑——池子裡本來就沒有那個 topic，balance 這層無事可做。
所以新增 `tools/cscs_quiz_direct.py` 的 `chapter_topics()` / `topic_spread_note()`：
算出每 topic 的目標區間寫進 prompt，並列出**相對於領先 topic** 落後的那幾個要求優先取材
（用全章 floor 當門檻的話，批次 2 會把 8 個 topic 全列成落後，等於沒講）。
重生成後是 `5/5/5/5/5/5/4/4`，38 題落在 38 個 item，那對 1.000 也一併消失。

**新學到三個生成器層級的坑**：

- **G19 對 `lang: en` 的題算「詞數」不是字數**（`len(why.split()) < 8`）。
  模型在英文題寫中文 `why_wrong` 時，中文沒有空白、整句算 1–4 個詞，
  於是以「錯因過短」的面目報錯——自己照 `len(why) < 12` 去掃會掃出零筆，
  而模型在 5 輪上限內也修不掉（它以為只要補長）。**G19 的詞數是當時唯一擋得住語言錯配的閘**；
  已在 `TOP_GATES` 補明文規則：`lang: en` 的 `stem`／選項 `text`／`why_wrong` 三者全英文。
- **批次 prompt 的結尾位置最有份量**。主題要求第一版放在 `TOP_GATES` 之後（最末），
  結果模型掉了 20 條候選裡 9 條的 `locator`、英文題掛零，該批只收得到 8 題。
  把它移到 `battery_note`／`TOP_GATES` **之前**（讓閘與禁用 item 清單留在最後）、
  並補一句「攤開主題不是放寬欄位要求」，同一批就回到 34 條候選、配額精準。
  **新增 prompt 區塊一律插在 `TOP_GATES` 之前。**
- **手改腳本的干擾項 dict 必須明寫 `'correct': False`**，否則 G1 報
  「選項 N 的 correct 必須是布林值」（它驗的是布林型別，不是「有沒有標對」）。

**人工逐題查證：38 題的正解全部落在自己的 `locator` 裡**——ch17／ch18 那個
「答案對但不在這條來源」的樣態沒有重演。抓到 6 條，都不是來源錯置：

| 題 | 病 |
|---|---|
| `core-concepts.i06.q1` | 干擾項「300 ms range, 600 to 800 ms range」句型和另兩項不平行，且錯因暗示 600–800 ms 不在教材裡，但 `rfd-impulse.i02` 的來源明寫絕對最大力量可能需 0.6–0.8 秒。改成「600 ms minimum, 0 to 800 ms range」並把錯因寫成真正的混淆點 |
| `rfd-impulse.i02.q1` | 題幹把數字寫成「零點六至零點八秒」，改回阿拉伯數字 |
| `core-concepts.i07.q1` | 題幹與 `i03.q1` 共用句型（0.600），重寫 |
| `sprint-technique.i03.q1` | 題幹與 `neuromuscular-basis.i08.q1` 共用「下列哪一項最正確」（0.571），重寫 |
| `sprint-speed-components.i02.q1` | 干擾項「1.85 m」的錯因寫「本章並未列出」——這種錯因教不了東西，改成指出誤把步幅當身高一半的想法 |
| `agility-cod.i06.q1` | 同義反覆：題幹描述 180 度側傾，正解把它換句話說一遍。改成真正要鑑別的「哪一組軀幹特徵最正確」，三個選項全換 |

**最後一條是 ch16–ch19 的新樣態**：題幹自己把答案講完了。
和 ch18 的「干擾項錯因供出反證」一樣，**這種病不必比對來源就看得出來**——
查證時先把題幹與正解並排讀一次，重疊的就是廢題。

改完跨題掃描的題幹撞擊全部歸零；剩 3 對正解相似度 0.500–0.667，
都是「同時間內產生較大力量」vs「快速力量產生」這種短字串被 `min()` 分母放大，與 ch18 同類，不擋。

### ch20：超額題數是重複的訊號；短題幹是碰撞磁鐵（2026-09-22）

**生成器修好後主題一次就攤平**：三批出完是 `6/5/5/5/5/5/5/5`，不必像 ch19 那樣重生成。
`topic_spread_note()` 到此驗證有效，ch21 起照用。

**但挑選器交出 41 題（超額 3 題），而 G10 只印不修**。
把多出來的 3 題當成「隨便砍三條湊數」是錯的——**超額本身就是模型把同一個考點出了兩次的訊號**，
挑選器沒有去重能力，它只會照 cognitive／topic 配額挑。實際查下去，多的正好是兩組真重複：

| 丟掉 | 原因 |
|---|---|
| `aerobic-performance-factors.i01.q2` | 與 `i02.q1` 題幹 0.848，兩題都問「V̇O₂max 相近但表現不同，下一步看什麼」、正解同為乳酸閾值。留 locator 就指在 Lactate Threshold 那節的 `i02.q1` |
| `periodization.i05.q1` | 與 `i03.q1` 考同一件事（賽前一天低強度短時間）。留英文那題以維持 13 題英文配額 |
| `special-issues.i08.q1` | 「減量研究多在哪類選手身上做」屬文獻族群 trivia，同主題已有減量期間與三種減量模型兩題 |

**丟之前先查重複，不要先湊數字**：三條剛好對上，是因為重複本來就是超額的來源。

**1.000 正解重複優先改寫、不優先丟棄**：`training-types.i04.q1` 的正解「高強度間歇訓練 HIIT」
與 `aerobic-performance-factors.i07.q1` 完全相同。但它的來源是 HIIT 的**處方參數**
（每週 1 次、工作段 30–90 秒、W:R 1:5、強度大於 V̇O₂max），原題一個參數都沒考到。
改成考 W:R（工作 60 秒配休息 300 秒）之後重複消失，**那條來源也才真的被用上**。
丟掉等於白白浪費一條沒被考過的來源。

**手改腳本的跨題斷言意外抓到一條來源錯置**：`program-design.i02.q1` 的正解
「任一變項每週增幅不超過 10%」與我新寫的 `periodization.i01.q1` 撞成 1.000——
追下去才發現它的 locator 是 Step 2: Training Frequency，那節只講頻率的決定因素
（強度×持續時間互動、訓練狀態、賽季、每週 >2 次才提升 V̇O₂max、>5 次傷害率較高），
**10% 進展上限根本不在裡面**，是 ch17／ch18 那個「答案對但不在這條來源」的老樣態。
改成考它自己那節的頻率研究。**跨題正解斷言不只防重複，它同時是來源錯置的偵測器。**

**短題幹是碰撞磁鐵**：`special-issues.i03.q1` 原本只有「依教材所述，減量訓練的典型期間為何？」
十幾個字，字元集太小，跨題掃描的 `min()` 分母讓它跟兩題毫不相干的題目都算出 0.556。
**修法是把題幹寫長（內容不變），不是每次繞過它**——否則它會一路污染 ch21 之後每一次掃描判讀。

**模型在 6 處 `why_wrong` 尾端留了落單的半形引號**（YAML 引號沒收乾淨的殘渣）。
22 道閘沒有一道看得到它，但它會直接印在題目畫面上。**每章收尾掃一次 `why_wrong` 的尾字元。**

人工查證 41 題全部讀完：**除上述之外，每題正解都落在自己的 `locator` 裡**。
剩餘掃描旗標 4 條題幹（0.556–0.587）都是「一名…選手…最應…」「A coach…」的情境骨架重複，
正解各不相同；正解旗標最高 0.833 是「23 至 28 下」對「138 至 151 下」這種數字串被
`min()` 分母放大，與 ch18／ch19 同類，不擋。

### ch21：來源的 D 註解就是模型會踩的坑；碰撞磁鐵可能是「題幹框架」本身（2026-09-22）

**生成器修好後第二章驗證**：三批出完正好 `5/5/5/5/5/5/4/4` = 38，零超額、零重生成。
ch19 的 `topic_spread_note()` 到此連兩章有效，ch22 起繼續照用。
生成成本：三批 55.5s／48.7s／181.3s（cache 81.8%／82.7%／88.8%），`--fix-only` 49.8s。

**批次過閘撞 5 輪上限是常態，不要在單批裡纏鬥**：批 1 殘 4 錯、批 3 殘 2 錯（都 exit 1），
但整章跑一次 `--fix-only` 就只剩 1 題收不動。**照既有節奏：批次撞牆就往下一批走，
整章 `--fix-only` 收尾**，模型在全章上下文下看得到的對照組比單批多。

**`--fix-only` 收不動的那題，根因是選項語彙本來就只有一組**：
`preparatory-period.i05.q1` 的「高強度搭配低至中訓練量」對「中強度搭配高訓練量」
G9 overlap 0.818——兩個選項都只能用強度／訓練量造句，五輪怎麼改都撞。
**這類題不要期待模型收斂，直接手改考點。**

**🔴 而手改時才發現那題的正解本身是錯的，錯在來源自己標的陷阱上**：
來源 D 註解明寫「『較低訓練量』是相對於肌肥大／肌力耐力相，而表 21.2 仍把此相的絕對組數
類別列為中至高」——這句話本來就是為了防止這個誤讀而寫的，模型照樣踩進去。
**逐題查證時把 `D` 欄當優先對照項，那裡常常就是模型會踩的那一步。**

**手改的第一版又撞到同 topic 相鄰 item**：改去考負荷處方（80–95% 1RM）後題幹與
`i04.q1` 撞 0.641，追下去發現 `i04.q1` 已經是「哪一組數字處方」題，
**而它的干擾項 2 就是我寫的那個正解**。改考來源另一條事實（這一相的動作會更具運動專項性）才過。
**同 topic 相鄰 item 的干擾項也要當成碰撞面掃過。**

**碰撞磁鐵可能是「題幹框架」而不是「題幹太短」**：ch20 學到短題幹要寫長，
但 `preparatory-period.i02.q1` 寫長之後仍是 0.564——因為
「根據教材對…的描述，最主要…為何？」這個**框架本身**就是磁鐵，加長只是加更多共用字。
**框架撞的時候加長無效，只能換框架**（改成「…在訓練量、強度與手段多樣性上的搭配，
最貼近下列哪一組描述？」）。

**正解撞到時，把變項名稱搬進題幹**：`competition-period.i04.q1` 與
`preparatory-period.i02.q1` 的正解都繞不開訓練量／強度／低這幾個字（0.500）。
解法是題幹寫「教練正在調整訓練量與強度這兩個變項」，正解就能縮成
「兩者一起壓得太低而流失適能」——**字面分開了，考點沒動。**

**兩組真重複，都用改寫不用丟棄**（ch21 零超額，沒有可丟的額度）：
`periodization-models.i06.q1` 是英文 `fitness-fatigue.i08.q1` 的嚴格子集
（跨 topic + 跨語言，ch20 同樣態），改去考 i08 沒考的那一層——**是哪一類疲勞**
（周邊與代謝）；`fitness-fatigue.i04.q1` 與 `competition-period.i04.q1` 都讀成
「賽前降負荷」，各自改回自己來源的獨有措辭。

**閘看不到 `why_wrong` 的句子通不通順**（G19 只數字數）：
`preparatory-period.i04.q1` 的「負荷與反覆次數範圍較低、較高反覆次數」過了 G19 但讀不通。
**每章收尾要把 `why_wrong` 整份讀過一遍**，連同 ch20 學到的尾端落單引號一起掃（本章 0 個）。

**英文題幹為了滿足 G15 硬塞 most/best 會造成問答不同調**：
`sfra-theory.i02.q1` 問「when must X be required」而正解是「not required」。
改寫成 `Which statement best reflects how … treats full recovery` 才對齊。

**數字型中文正解跨題會在 `min()` 分母下爆 1.000**：短字串的字元集是長字串的子集
（「每動作 1 至 3 組、每組 1 至 3 次」⊂「50–75% 1RM，每動作 3 至 6 組、每組 8 至 20 次」）。
**數字選項只在同題內安全**（G9 有 `differ_only_in_numbers` 豁免），跨題沒有豁免。

**手改腳本改用三集合，比 ch20 的 `STEM_ONLY` 精確**（`ch21_polish.py` 可當範本）：
`NEW_STEM`（只動題幹 → 跑 G15 + 跨題題幹斷言）／
`NEW_OPTS`（動了選項 → 跑完整選項閘 G1–G5、G9、G16、G19、G20）／
`NEW_ANSWER`（正解文字真的換掉 → 才跑跨題正解斷言）。
ch20 的兩集合寫法會對「只改干擾項」的題目誤跑正解斷言。

剩餘 1 條掃描旗標：`competition-period.i05.q1`「漸進強度並搭配極低訓練量」對
`preparatory-period.i02.q1`「高訓練量、低強度與多樣化訓練手段」0.500——**語意相反**
但共用訓練量／強度／低，與 ch18–ch20 同屬 `min()` 分母放大的可接受類別，不擋。

### ch22：短名詞章讓 G2／G4 幾乎無法同時滿足；22 道閘看不見中文半形標點（2026-09-22）

**ch22 是第一個殘錯破 10 條的章**（`cf55c2e`）。生成本身沒退步：配題一次命中
`(38, 3, 18, 17, 13)`、8 個 topic 拿到 5/5/5/5/5/5/4/4、零超額，
ch19 的 `topic_spread_note()` 到此連三章有效，ch23 起繼續照用。
成本：三批 127.3s／82.4s（全綠）／139.6s，`--fix-only` 61.5s，cache 78–98%。

**殘錯破 10 條的根因是本章正解的詞性，不是模型退步**：復健章的正解大量是
3–6 字的中文短名詞（期別名、解剖位置、角色名如「適應症」「腱實質」），
**G2 的 ±15% 與 G4 的 1.5× 在這個長度幾乎無法同時滿足**——「適應症」3 字要配
一個 3–4 字的干擾項，一旦寫得像真誤解就超長。兩批都燒完 5 輪就是這個原因。
遇到這種章不要讓模型硬撞，直接把三個選項對齊成同一句式（「列為適應症」／
「列為禁忌症」／「屬預防性措施」），長度自然收斂。

**中文數字選項拿不到 `differ_only_in_numbers` 豁免**：「第一型／第二型／第三型膠原蛋白」
三對全撞 0.750，模型五輪必死。該函式只剝阿拉伯數字，`一二三` 是普通漢字。
ch21 記的是阿拉伯數字版本（那會被豁免），**這是相反的中文數字版本**。
修法是讓三個選項不要都以「第 N 型」起手。

**22 道閘全都看不見中文句子裡的半形逗號與分號**（本章 12 處）：G16 只擋選項 `text` 的
分號，題幹與 `why_wrong` 的 `,` `;` 過得了全部閘卻印錯。收尾要跑一次
**只在緊鄰漢字時才替換**的正規化（`ch22_polish.py` 的 `fix_punct()`）——不能無條件換，
否則數字千分位與英文括號內容會一起壞掉。先確認 zh 選項 `text` 零半形標點，
`;`→`；` 才不會反過來踩 G16。

**兩類閘完全看不到的內容缺陷，只有逐題對照來源才抓得出**：

- **來源列 N 個等價答案時，「最可能」型題幹必然有多個正解**。`injury-types.i06.q1`
  問跟腱「最可能的失敗位置」，來源列了肌腹／肌肉肌腱交界／肌腱附著骨處三個等價位置，
  兩個選項同時對。**翻成「最不可能」就只剩腱實質一個答案**。
  `injury-prevention.i03.q1` 是同一個陷阱的變體：三個等價風險因子被問成「最應優先處理」，
  來源沒有這個排序——改問「是否屬於本章所列」，收斂靠的是干擾項都不在清單上。
- **正解抄回題幹用語＝自問自答**。`injury-prevention.i06.q1` 題幹寫「選擇下肢傷害預防的
  結構化計畫時」、正解就是「結構化預防計畫」，而它真正的來源是 Sportsmetrics 與 PEP，
  題目從頭到尾沒碰。逐題查證時要**比對題幹與正解的用詞重疊**，閘不會報。

**同 item 兩題 + 鄰近 item 一題可能三題同考點**：`maturation-phase` 的 i03.q1／i06.q1／
i06.q2 都在考「一般 → 運動專項」。`--redo` 就是為這個設計的。

**題幹框架磁鐵在 ch22 又復發一次（ch21 教訓）**：`resistance-training-systems.i04.q1`
第一次改寫保留了 DAPRE 情境框架，仍對 `i03.q1` 撞 0.59——**框架本身是磁鐵時寫長沒用**，
要換考點（改問第四組阻力憑什麼決定）。反過來，`kinetic-chain.i01.q1` 那種
19 字的短題幹是純長度磁鐵（撞兩題），內容不動只寫長就解決。

**手改後的跨題斷言會出現機械假陽性**：`min()` 分母遇到兩字正解會失真——
新正解 `The repetitions completed on set three` 對 `Third degree` 算出 0.5，
只因為共用一個 `third` 而後者只有 2 個詞。這種不是真重複，換掉那個詞即可。
同理 `sports-medicine-team.i07.q1` 改寫後撞上只有 25 字的 `kinetic-chain.i04.q1`，
共用的全是「選手／下列／最適合／訓練」這種通用框架詞——**改寫時順手避開通用框架用語**，
比事後調閘划算。

**助理自身的併發錯誤（流程教訓）**：Bash 工具會自動把長指令轉背景，
**不要因為看不到輸出就重發一次**；兩個同名生成程序並行會交錯寫同一份 yaml。
發現就 `taskkill` 砍掉、刪掉 yaml 重跑，不要搶救——而且選擇器每次挑的 item 不同，
污染那次的殘錯數（1 條）根本不能拿來當基準。等背景指令用 `run_in_background`，
不要用 `sleep`（開頭的長 `sleep` 會被工具直接擋掉）。

### ch23：邊際命中不等於配額格命中；捏造指涉是 recall 章最大的隱形缺陷（2026-09-22）

**ch23 完成 40 題並已逐題查證**（`340da94`）。過閘 0 錯、跨題掃描題幹 0 撞擊，
人工查證改寫 24 處。成本：三批 + 一次 `--fix-only`，MiniMax 5H 9%／7D 2%。

**`(40, 28, 12, 0, 13)` 這種配題表要盯「配額格」不是兩條邊際**：三批生成把
recall 28／application 12 與 en 13 兩條邊際都算對了，但 `target_grid()` 展開的
四格聯合目標是 **recall/en 9、recall/zh 19、application/en 4、application/zh 8**，
實際交出 recall/en 10、application/zh 9、application/en 3。`--fix-only` 補了一題
recall/zh 卻補不出 application/en，又不肯砍題（砍了 application/en 更無法滿足），
於是卡在 41 題。**這不是工具 bug**，是生成階段沒把 cognitive × lang 的乘積講清楚。
修法是**一步同時動兩格**：把一題 recall/en 改寫成情境題變 application/en，
recall/en 10→9 與 application/en 3→4 同時解決，內容一條都沒丟。
**ch24 配題表完全相同，預期會再遇一次**——生成 prompt 要直接列四格數字。

**recall 佔七成的章，題幹框架磁鐵是叢發不是個案**：一次掃出 9 條題幹撞擊。
共同分母是中文的「肌力與體能設施」＋「最…為何？」與英文的
"in a strength and conditioning facility"——同一個框架被 28 道 recall 題輪流套用。
**改寫要挑「共同分母那一題」**：`design-elements.i01.q2` 一題重寫就清掉 3 對。

**捏造指涉的 `why_wrong` 是本章人工查證的最大類（24 處裡約 14 處）**，而且
22 道閘一條都看不到——G5 只比對 `why_wrong` 與**它自己那個選項**的字面重疊，
G19 只管長度。實例：說 1:6 是「ADA 的標準比例」（ADA 也是 1:12，跟課本同值，
本章的 ADA 只出現在無障礙電話）；把 12 in 說成鏡面到器材的淨距（那是 6 in，
12 in 是坡道水平長）；替導桿發明月／季／年度保養頻率（圖 23.6 沒有）；
把乙烯基軟墊、隔音護欄、有色玻璃當成教材列出的選項（都不在清單裡）。
**recall 章的干擾項全靠同章事實對照，模型最容易在這裡編一個聽起來合理的出處。**

**懷疑是捏造時要 grep 整章來源，不能只看該題自己的 item**：我判成捏造的三條
後來全被來源推翻——8 ft 真的是表 23.2 的平台高度、「施工期通常最長」真的寫在
`four-phases.i01`、「停滯空氣造成異味」真的寫在 `environment.i04`。
跨 item 引用同章事實是合法的，先 grep 再改。

**跨 item 借數字是另一類閘看不見的錯**：`layout.i01.q1` 正解寫了「49 ft²」，
但 `ch23.layout.i01` 的 `numbers` 是空的——那個數字是從本章空間標準的另一條借來的。
**改題順手看一眼該 item 的 `numbers`**，沒有就不該出現在選項裡。

**選項互相否證＝不用讀書就能刪**：`needs-analysis.i01.q1` 的選項 3
「同時使用人數與訓練品質無關」直接否定選項 1，選項 2 也是在評論正解而不是
另一個平行答案。這種整組重建，三個選項要是三件**平行的事**。
同章另有一個送分選項（「因為訓練設施不需要進行接地」）——沒人會選，
`why_wrong` 還在講「違反安全基本要求」而不是診斷誤解，一併換掉。

**topic 分布 7/7/5/5/4/4/4/4，沒有任何閘擋這個**（ch20–ch22 都是平的
5/5/5/5/5/5/4/4）。`select_questions` 只能在模型交出的候選池裡排，池子本身偏斜
就攤不平；`topic_spread_note()` 是 prompt 端的唯一防線。每章生成後先看這一欄，
偏斜就直接 `--redo` 補偏少的 topic，不要留到人工查證階段。
（後續：這條防線已在 ch24 前收成 ±1，ch24 因此第一次自動攤平，見下節。）

### ch24：閘看不見的三類缺陷，與「模型不會說自己修不動」（2026-09-22）

**ch24 完成 40 題並已逐題查證**（`b379c4d`）。22 道閘 0 錯、跨題掃描各類別全 0，
人工查證改寫 19 處。配額格四格全中（19／9／8／4），`--fix-only` 一次就收斂，
沒有重演 ch23 那種卡 41 題的狀況。

**`topic_spread_note()` 收成 ±1 之後，topic 第一次不靠人工就攤平**：
8 個 topic 各 5 題、40 個 item 零重複。舊的 `-1/+2` 區間對 40 題 8 topic 算出來是
4–7，ch23 的 7/7/5/5/4/4/4/4 **完全落在區間內**——那等於沒有約束。
改成 `floor, ceiling = max(1, round(target) - 1), round(target) + 1` 才真的咬得住。

**⚠ 我在 `75923b9` 引進一個按批次算配額格的加總 bug，尚未修**：
`target_grid()` 是在 `quota_for(chid, part)` 切出來的**單批配額**上跑最大餘額法，
而每批各自進位不會加總回全章的四格目標——ch24 三批算出 10／18／3／9，
全章目標是 9／19／4／8。`run_chapter:1519` 的 `grid = target_grid(quota)`
同時餵給 `select_questions`，所以選擇器執行的是那個漂掉的格子。
**暫不修的理由**：(a) `--fix-only` 會自己收斂回全章目標；(b) 剩下的生成工作
（ch01–ch03 追加）是單批 append，不帶 `--part`，碰不到這條路徑。
要修就是把全章 grid 算好再按批減，不要在批內重算。

**模型修不動的題會從 patch 回覆裡「無聲消失」，五輪全燒掉**：
`emergency.i07.q1` 三個選項文字**字面完全相同**（都是「每季至少一次」，
G9 三對都是 1.000），跑完 5 輪 `MAX_FIX_ROUNDS` 一次都沒被碰。
根因不是工具 bug——該 item 的來源只有一個頻率值，模型沒有干擾項素材，
於是每輪都把這題**整題略過**而不是回報修不動。
**看到某題連續多輪殘留就直接人工接手，不要再加輪次。**
修法是從它自己的 `why_wrong` 把它原本想寫的干擾項撈回來
（「放寬到兩年一次」「只在新進人員到職時」），補成三個平行選項。

**正解與來源相反是本章最嚴重的一條，22 道閘全部看不到**：
`legal.i07.q1` 標成正解的選項寫「診斷或評估醫療狀況並開立醫療放行」，
但來源明寫執業範圍**不**包括診斷、醫療放行**僅**運動醫學人員能開——
三個選項裡沒有一個對。**recall 章逐題查證時要先對來源驗正解，再看干擾項**，
順序反過來會被三個看起來合理的干擾項帶走注意力。

**`why_wrong` 裡對著出題者說話是第三類**：「本題把它當作干擾項，但答案要的是
『不包含』」「把它列為干擾是為了與『不張貼』對照」——這是寫給改題的人看的
後設註解，不是給讀者的內容解釋。G5 只比對它與自己那個選項的字面重疊、
G19 只管長度，兩道都放行。**掃 `why_wrong` 時看它在對誰說話。**

**「最完整反映…」這種題幹形式在結構上保證 G2 失敗**：要求「最完整」就等於
要求正解比干擾項多講一層，正解必然變長，而 G2 是
`正解長度 * 100 > 次長 * 115` 就擋。這類題幹一律重寫成問單一事實。

**題幹框架支配是整章現象，不是個別撞擊**：第一次 patch 後跨題掃描還剩 7 條題幹旗標，
逐對去追都追不乾淨。改去數框架才看出真相——27 道中文題裡 **10 道以「為何？」收尾、
8 道含「最主要」、7 道用「下列哪一項／何者」**，整章像同一個模子。
一次改寫 14 道題幹（`ch24_stems.py`，只動框架不動內容、正解沿用）後旗標歸零，
框架計數降到 2／1／3。**題幹旗標超過 3 條就別逐對修，去數框架。**

另外，跨題掃描用 **min() 分母**（比閘的 union 嚴），對**帶括號英文原文的題幹**
會機械性誤報——「產品責任（product liability）」這類註解讓兩題共用一串拉丁字母。
實測把註解剝掉後會**換出另外 7 條**旗標，證明兩種成因同時存在：
**先剝註解重掃一次，剩下的才是真的框架重複。**

**流程外的一件事**：推送時撞到 non-fast-forward——另一個 session 已推了 `7d4b329`。
那是同一批 Nordin 章節指南工作的另一個 hash（它以遠端為基底重挑 13 檔，
刻意不含我的 `75923b9` 與未追蹤的 ch24）。**用 merge 接回，不 rebase 不 force**，
`de13bda` 推上去後兩條線收斂。兩份文件的衝突區都寫著「已備妥但尚未推送、待使用者回覆」，
那個描述在合併當下已經過期，已在 `HANDOFF.md` 與該研究目錄的 `RUN.md` 各補一段明標更正。

### ch01–ch03 追加題：三章補齊，學到的三件事（2026-09-22）

**ch01 的失敗模式是「每輪都交修正、但永遠不收斂」**：模型照著閘的錯誤訊息改，
改完換一批新錯，五輪跑滿還在原地。已加停滯偵測——`previous = frozenset(errors)`，
**本輪錯誤與上一輪完全相同就提前停手交人工**，不再燒滿五輪。ch03 在第 4 輪觸發，
省下一輪；這是它第二次驗證有效。

**ch02 的修正輪把兩道英文題翻成了中文**。`lang` / `cognitive` / `item` 是配題格子的
座標，一翻就等於把格子搬走，而 G13（格子計數）是**只印不修**的章級檢查，擋不住。
已在 `apply_patch` 加 `LOCKED = ("cognitive", "lang", "item")`，修正輪不准動這三欄。

**ch03 有兩個新教訓。** 一是**簡體字漏進題庫**（氧亏 ×5、同位选手 ×1），閘的
G11「語料未見字」只是警告不是錯誤，而且它同時會印出 倚兇莫革 這類完全合法的繁體字，
所以那行紅字不能掃過去，要逐字看。二是**跨題掃描的 min() 分母會讓「把答案縮短」
反而撞得更兇**：phosphagen.i03 的正解縮成「同時產出 ATP 和 AMP」只剩 10 個字，
光 `ATP` 加一個空格就供得起一半重疊，對舊題的相似度從 0.5 漲到 0.7。
**正解撞題要往長寫、換掉共用字，不是往短寫。**

**人工查證抓到一題考的正好是書中明說不成立的事**：`substrate.i04` 問「哪個強度區間
造成最大肌糖原耗竭」，而它自己的來源條目最後一句寫著「總工作量相等時，阻力訓練的
糖原耗竭總量不受強度影響」。閘和掃描都看不到這種事——**逐題查證時要把來源條目
整條讀完，不只讀正解對應的那一句。**

**24 章連同 ch01–ch03 追加題全部出完。** 下一步見下節（舊題庫合併已完成）。
另有待辦：本庫沒有常模表，spec R7 目前全面禁止百分位數字，R6 的測驗組合題因此少一段。

### 舊題庫合併：287 題進來 66 題，以及兩個該由程式擋的缺陷（2026-09-22，`d5f77a4`）

`C:/claudehome/resources/raw/notes/CSCS_Full_QuestionBank.md` 的 287 題已全部裁決完畢並
併入，原檔按 `resources/CLAUDE.md` 的規則已刪除。工具是
`tools/cscs_quiz_legacy_merge.py`（`--retrieve` 建候選、`--dry-run` 看 prompt、`--apply`
寫入、`--fix` 修閘錯）。

**產出帳**：287 題 → 6 題在發問前就被擋掉（全庫沒有相近條目，最佳候選低於 0.20）→
281 筆裁決，94 接受 / 187 拒絕（沒有教材依據 134、與既有題重複 32、題目本身站不住 12、
該條目已滿 8、無法測 1）→ `--apply` 實寫 70 題（24 題卡在每條目最多兩題或題幹已存在）
→ 閘與人工查證再刪 11 題、改寫 7 題 → **最終留下 66 題**。淨收率約 23%，比試點外推的
112 題低不少，主因是**條目上限與跨題掃描的重複淘汰比預期吃得深**。

**兩個防線寫進程式而不是寫進 prompt**，理由是這兩件事是抄寫與計數，不是判斷：

- **`locator` 一律從 `data/cscs/chNN.yaml` 機械回填**，不採信裁決裡那一行。G7 要求與來源
  條目逐字相同，模型卻會自己補「第 8 章 ·」這種前綴或改寫階層——實測撞了 2 題。
- **題幹已在題庫就不寫第二次**。試點那 20 題是先手動併進題庫才跑全量裁決的，裁決檔仍留著
  它們的 accept，重跑 `--apply` 會原樣再生一題（實測撞 4 題 G8）。加了 per-chapter 的
  `stems` 集合之後，重跑正好丟掉那 7 題試點題。

**閘全綠之後，跨題掃描與人工查證又刪掉 11 題、改寫 1 個選項**。三題直接牴觸教材：
`ch17.needs-analysis.i04.q2` 把三個月訓練年齡判成「未滿兩個月的初學者」（教材是
初學 <2 個月、中階 2–6 個月）；`ch13.anaerobic-endurance.i02.q2` 把 300 碼折返跑說成
最大無氧功率測驗（同章 i01 明寫它與只持續數秒的最大無氧功率測驗不同）；
`ch22.maturation-phase.i03.q2` 把運動專項訓練排在組織耐受度重建之後（教材是本期就加入）。
另外 `ch13.anaerobic-endurance.i01.q2` 的正解寫「約五分鐘的做功」——五分鐘是兩趟之間的
休息。還有兩題的正確選項上掛著字面 `omit`，那是 prompt 裡的指示字被當成內容寫了進來。

**結論與前 24 章一致**：模型判定「接受」的題目裡，大約每六題就有一題帶著閘看不到的缺陷
（掛錯條目、題幹自問自答、與教材牴觸、數字理由對調、洩漏指示字）。**人工逐題查證是
結構性必要，不是可選步驟。**

**接 cscs-quest 的注意事項**：`cscs_quest/catalog.py` 從沒載過 `_quiz_bank`，它的
`validate_question` 是另一套 schema（`kind` 只收 recall/choice、2–8 個選項、`correct`
是整數索引）。要加的是第三個載入器，仿 `_load_book_questions()` 的寫法，產出 `bank:`
開頭的 `kind: choice` 題；UI 上 `pool: exam` 是模擬卷、`pool: extra` 是額外練習，要分開呈現。

## 兩冊33章詳解加深完成：骨折固定與人工關節已部署（2026-09-22）

沿使用者接續兩冊肌肉骨骼繁中讀本及既有編輯／發布授權，完成最後待加深的Nordin第15章27節／5,886中文字、第16章37節／8,120字；本批64節／14,006字，原八錨點保留。全讀本33章、1,106節／260,914字，Neumann1–16及Nordin1–17均已加深，仍為guide／chapter-guide；不等同逐句全譯、附錄全譯或全部原研究精校。

主代理核閱413–419、421–438主要正文，回看11張原PNG，另核六篇原始研究摘要。釐清骨折間隙應變、骨板EI、髓外／體外固定、動態慣性、肌肉拉力限制、側傾兩種力臂、步數／單側週期及兩組件合計旋轉。未沿用來源不明確的400倍風險、人工膝8BW常模、樓梯全部分量一律增30%等概括。來源、22組語意決策及剩餘界線見`research/book-readers-2026-09-06/fixation-arthroplasty-expansion/RUN.md`。

Hugo172頁、HTML1,650連結／錨點、既有閱讀器97/97、原頁服務18/18、本批71/71通過；六張手機紙色／夜色與桌面截圖已目視。五份共用版型／CSS／JS、來源catalog及其餘31章與基線75923b9一致，證據在同目錄`validation.json`及`browser-checks.json`。8768本機唯讀預覽可用，原PDF、抽文與原PNG留本機。

**同一批工作有兩個 hash，不是兩批內容**：本機實作`7d1c435`（基底含CSCS提交75923b9），另以遠端`6c4f9f5`為基底挑出相同13檔形成`7d4b329`。當時自動核准審查拒絕推送、交接檔記為「已備妥但尚未推送、待使用者回覆」——**該描述已過期**：`7d4b329`其後已進遠端`hugo-source`。兩者13檔內容一致，唯一差異是基底是否含`75923b9`；2026-09-22 的CSCS批次把遠端合併回主線後，兩條線已收斂，不需再挑檔或改寫`75923b9`。使用者明確回覆「推送吧」後，提交`7d4b329e0567094378b9ffa1cf92f8fca7a9e0d3`已成功部署；完整SHA對應[CI 35704841803](https://github.com/Hangsau/cortex/actions/runs/35704841803)的build／deploy均成功。正式站30/30通過，兩章全文、27／37節順序、原八錨點、跨章目標、原頁群隱藏及CSS／JS均與驗收版一致。證據見同目錄`ci-verification.json`、`public-validation.json`及`publication-status.json`；先前自動核准拒絕已由使用者明確授權解除。

已無尚未加深的主章。下方舊「下一批15–16」與更早待辦均為歷史快照。開始時另有未追蹤的`data/cscs/_quiz_bank/ch24.yaml`及未推送的既有CSCS提交75923b9；本批不納入其內容發布，使用獨立工作目錄交付教材變更。
...
## Nordin 關節軟骨、肌腱與韌帶詳解已部署（2026-09-21）

沿使用者「繼續」及既有編輯／發布授權，完成Nordin第3章41節／7,680中文字、第4章42節／7,861字，本批83節／15,541字；原八錨點保留。全讀本33章、982節／232,792字，Neumann1–16及Nordin1–4、7–14、17已加深，仍為guide／chapter-guide。

本批由主代理核閱撰稿，未派其他模型。核閱主要正文77–109、119–136及11張原PNG，辨明雙相／三相、平衡／暫態壓力、摩擦面／排液面、結構／材料曲線與應變率／重複頻率。第86頁厚度時間反比以原始理論式校正，第126–127頁剛度／能量單位依座標量綱修正；第129頁原印刷也有固定量錯誤，REVIEW更正先前全歸因於OCR的說明。編者算例已獨立回算；治療、藥物、修復與培養數據保留歷史及物種界線。

Hugo172頁、HTML1,508連結／錨點、既有閱讀器97/97、原頁服務18/18、本批閱讀／樣式58/58通過。目視兩章手機兩色及桌面；公式行內code在夜色不清楚，改正文strong後重建與重驗58/58。五份共用版型／CSS／JS與基線8312a40一致。來源及驗收見`research/book-readers-2026-09-06/cartilage-tendon-expansion/{RUN.md,validation.json,browser-checks.json}`。實作`8d0dc1d9dd4c1e30be3a1a155c12dc018bfbfe52`已部署，完整SHA對應[CI 35573939823](https://github.com/Hangsau/cortex/actions/runs/35573939823)的build／deploy均成功。正式站21/21：兩章全文、41／42節順序、原八錨點、跨章落點、隱藏原頁群及CSS／JS與本機一致；證據見同目錄`ci-verification.json`與`public-validation.json`。

下一批接Nordin第5章周邊神經、第6章骨骼肌，再續15–16章。已有詳解的章不需重寫。8768本機唯讀預覽仍可閱讀。保留既有風格及發布授權；原PDF、抽文頁包、PNG、截圖留本機。下方為歷史快照，勿使用舊「下一批3–4章」取代本段。

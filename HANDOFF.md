...
**⚠ `related` 空白是跨章既有債，不是 ch17 個案（2026-08-01 掃描更正）**：先前記為「只有 ch17 全空、其他章 37–62 條」是錯的。實際覆蓋率——ch01/ch03/ch09 100%、ch02/ch13 96%、ch11 87%、ch10 79%、ch16 69%、ch12 67%、ch14 62%、ch15 51%、**ch05 39%、ch08 6%、ch04／ch06／ch07 全 0%**；**ch19 雖然 64/64 有值但只有 11% 跨章，等同章內湊數，一併列入**。**這六章（ch04/ch06/ch07/ch08/ch05/ch19）要另排一輪 `related` 補完**，直接複用 `.prompts/ch17-related.md` 這份窄範圍範本（改章號即可，成本低、範圍安全）。範本已驗證有效：只動 `related`、其餘欄位逐位元凍結。

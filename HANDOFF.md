...
- **⚠ ch01 的 `cscs_check` 顯示 OK，但不可信**：68 條欄位雖填滿，同樣是憑記憶補的。已確認至少兩個數字書中不存在——軸心骨「約 80 塊」、附肢骨「約 126 塊」（解剖學上正確，但這本書沒講），且兩者都掛著 `locator: 第 1 章 · The Skeletal System`。**假引用比空白更糟**：空白會被查，假引用讓人以為已經查過。用「數字字串有沒有出現在該章」做機械掃描會給出 26/27 的假通過率（「80」是靠書中「竇房結 60–80 次/分」蒙混），只有讀原文才驗得出來。
- **真實進度**：對照原文完成 **1 個 topic**（ch05 結締組織）／欄位滿但待對帳 **8 個 topic**（ch01）／完全空白 **193 個 topic**。全書共 202 個 topic。

**下一步建議**：① **對帳式補完是唯一剩下的大工程（201 個 topic）**——每個 agent 認一章、自己讀 `resources/books/.../chNN_*.md`、一個 topic 填完立刻寫盤，每章跑綠 `cscs_check` 才 commit；ch01 走同一條流程但工作是「核對既有內容 + 修正 + 把 locator 改成真的指得到的位置」，不是從零填；②（配額）2026-07-31 當下 Claude 5H 已用 83%、Codex 與 MiniMax 週配額均 100% 用盡（分別約 5 天、2.6 天後回），照 5H 規則窗尾不開大批次，須等重置後串行派發；③ 遮答自測仍是全頁 toggle，「逐條遮／已答對不再遮」需要 localStorage 層，未做；④ `layouts/library/book.html` 與書庫其他 layout（library.css / bookshelf.css）調性未對齊。

...
- **2026-09-26 跨系列知識連結（進行中）**：使用者要求「能連的就要連、沒有的不硬連」，重點是 CSCS ↔ 肌動學 ↔ 基礎生物力學，以及 Vortex 通到這三本的地方。`tools/crosslink_candidates.py` 把讀本 1,106 小節、CSCS 1,557 條、Vortex 404 單位（技術分析、傷害、動作圖譜、呼吸、週期化）用字元二元組 TF-IDF 找候選（`--min 0.05 --top 4`，3,367 對，產物 `next/crosslinks/{units.json,candidates.jsonl}`）；`tools/crosslink_judge.py` 直呼 MiniMax-M3 逐對判斷「同一具體現象且讀了有實質補充」才 link=true，結果逐批寫 `next/crosslinks/judgments.jsonl`（可續跑，撞額度 exit 2）。試點 40 對（最高分段）全判相關、理由正確；**全量跑完後必須抽查低分段是否會判 false，太寬就收緊 prompt 重判低分段**。下一步：呈現——讀本在每個小節標題下列相關（render-heading hook）、CSCS 條目加「其他書」、Vortex 接著看加「教科書怎麼說」

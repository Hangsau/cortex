...
- **P5 正式切換尚未執行（等使用者授權）**：刪除舊 `layouts/`、`static/` 被權限審查以「不可逆本機刪除」擋下，改只動 `deploy.yml` 的做法也被視為同一結果而擋下。使用者授權後的步驟：① `deploy.yml` 的 Build 改為 `pip install pyyaml` → `python3 tools/build_vortex_links.py` → `python3 tools/build_library.py` → `python3 tools/check_learning_map.py` → `hugo --minify --config hugo.next.toml --baseURL https://hangsau.github.io/cortex/ -d public`，並移除「Build next preview」步驟；② 驗線上首頁與 10 個轉址；③（可選，需使用者同意刪除）`layouts/shortcodes` 移到 `next/layouts/_shortcodes`、刪 `layouts/` `static/` `data/home.yaml` 與舊 Playwright 稽核（`tools/{home_audit,audit,audit_book_readers,audit_learning_map,audit_periodization_planner,audit_vortex_problems}.js`），`hugo.next.toml` 改名 `hugo.toml`（baseURL `/cortex/`、publishDir `public`），CLAUDE.md 的舊版型規則段改寫為新架構

...
- **2026-09-26 書房「今天的短篇」改為每日輪替**：原本寫死四個入口、讀過就整區隱藏。現在 `tools/build_starters.py`（需先跑 `build_library.py`）產 `next/data/starters.json`（讀本每章第 1、4、7…節 381 個、CSCS 每主題第一條 198 個、學習技法＋課堂策略 38 個、氣質 14 個，118 KB），首頁以 `resources.FromString` 發佈、`study.js` 依日期每組挑一個、優先沒讀過的，「換一組」可重抽；有紀錄時標題改「想換個口味？今天的短篇」且不隱藏。讀本或 CSCS 內容變動後要重跑這兩支

...
## 兩冊33章詳解加深完成：骨折固定與人工關節已驗收（2026-09-22）
...
**同一批工作有兩個 hash，不是兩批內容**：本機實作`7d1c435`（基底含CSCS提交75923b9），另以遠端`6c4f9f5`為基底挑出相同13檔形成`7d4b329`。當時自動核准審查拒絕推送、交接檔記為「已備妥但尚未推送、待使用者回覆」——**該描述已過期**：`7d4b329`其後已進遠端`hugo-source`。兩者13檔內容一致，唯一差異是基底是否含`75923b9`；2026-09-22 的CSCS批次把遠端合併回主線後，兩條線已收斂，不需再挑檔或改寫`75923b9`。本段不代為聲稱CI或正式站驗收，該批次的接續證據見同目錄`publication-status.json`及RUN。

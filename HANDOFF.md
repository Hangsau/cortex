...
| ch15 | 58 條 / 0% 跨章 | 229 條 / **84.3%** | 12 | `6a945af` || ch16 | 51 條 / 0% 跨章 | 106 條 / **94.3%** | 12 | `2c411d8` |
...
**下一步建議**：① **`related` 第二輪補底（11 章）**——`ch01` `ch02` `ch03` `ch09` `ch10` `ch11` `ch12` `ch13` `ch14` `ch15` `ch16`，全部是 100% 章內互指、跨章 0 條。用 `.prompts/chNN-related.md` 這套窄範本（第一輪七章已驗證：只動 `related`、其餘欄位與 cards 逐位元凍結），但**鄰居清單必須按該章在全書的角色重寫，不能只改章號**（第一輪的教訓，見上）。ch15／ch14／ch12／ch16／ch10 覆蓋率也偏低要一起補。串行派發，一章驗收 + commit + push 才派下一章。② **欠 ch01–ch03 一次卡片補課**：這三章的卡片只做過點狀修補（`33d7df5` 修了 ch02 三張），從沒回源書整批重寫；ch13/ch15/ch21/ch23/ch24 已在對帳時重寫過，可跳過。③ `ch17.principles-exam-tips` topic id 改名（36 條入向連結要同步改），等你決定要不要做。④ 遮答自測仍是全頁 toggle，「逐條遮／已答對不再遮」需要 localStorage 層，未做。⑤ `layouts/library/book.html` 與書庫其他 layout（library.css / bookshelf.css）調性未對齊。⑥（配額）5H >80% 一律不開新批次，排 shotclock 等下一窗。

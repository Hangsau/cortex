...
- **🔴 下一步是第四輪，範圍比原估大很多且目前卡在配額**：剩下 18 章 2490 條連結，其中 10 章的分佈是 `{2: 64}`（每條 item 剛好 2 個、無一例外）＝下限被逐條踩滿的直接證據。原本寫的「約 5% 湊數」是抽樣過小的低估，**已作廢**；校正後預期各砍 30–45%。**codex 7D 93%（約 6 天後重置）、minimax 7D 100%（約 19 小時後重置）**，兩條線都派不動。範圍表與派工順序見「🔴 第四輪的範圍比原本估的大很多」一節。
- **常設限制**：站主指示 Opus 只做規劃／派工／驗收，**不用 Claude 額度生內容**。第四輪要等外部額度。

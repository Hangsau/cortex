...
## CSCS 題庫重寫：ch01–ch05 完成，配題表已上調 2.5 倍（2026-09-14）
...
10 recall / 6 analysis，12 EN / 23 ZH，19 道閘 0 錯）。ch06–ch24 尚未完成，照章號串行，
...
一支窮舉腳本掃候選組合、同時算 G2/G3/G4/G9 再挑，比逐句試快得多。三條已驗證的推論：
**英文選項超過 ~35 字元就會收斂到整個字母表、必定超標**（目標 24–32 字元）；
**「屬性配對型」選項（`Type II fibers, limiting hypertrophy` 這種兩個名詞並置）在結構上
無解**，要改題幹的提問角度讓選項變成彼此無關的單一陳述；`why_wrong` 撞 G5 時，
加入選項裡沒有的字元（數字、大寫、`;`、`:`）就能壓下去（含冒號的字串記得用 YAML 單引號）。

**接下來**：**下一章是 ch06**，`.prompts/ch06-quiz.md` 已生成（35 題＝10/20/5，英文 13）。
發包一律帶 `--settings tools/cscs_quiz_delegate_settings.json`——那是 git 禁令閘。
ch06 → ch24 走完（若 M3 在單次發包內寫不完 35 題，再考慮加 `--part K/N` 的分批機制；

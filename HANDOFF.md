...
**Vortex `/vortex/` 首頁瘦身（2026-06-20，commit 1ae1daa，push hugo-source + CI run 27859278392）**：站主回饋首頁「真的有夠難讀、為什麼一頁拉這個長」。本機 playwright 截圖量到全頁高 **5853px（約 6.5 螢幕，遠超 DESIGN_SYSTEM 鐵則 A 的 3 螢幕上限）**。問題不是配色（已是博物館說明牌風），是**垂直留白過量 + 每個 section 都有冗長鋪陳段**。只改 `static/css/vortex-techo.css` + `layouts/vortex/vortex-home.html` 兩檔（38 增 38 刪）：①`--tx-air` clamp 64–132px → 44–78px；masthead/section-head/hero/path/ledger/card 留白與地圖節點 min-height 96→74 全砍約四成；巨型標題 clamp 72–188 → 56–116px。②每個冗長鋪陳段改倒金字塔單句（鐵則 B 段首承載資訊）：masthead lead、hero-d、地圖 note、ledger note 都從「零資訊起手的整段」收成一句結論。**未動配色、未動博物館美學、未拆 IA**（單頁知識模型保留）。**驗收**：本機 `hugo --baseURL localhost:8099 --destination <tmp>` build → `python -m http.server` → playwright headless 截圖前後對照，全頁高 5853px → **4561px（-22%）**，prose 從文字牆變可掃讀。⚠ 仍是 ~5 螢幕（地圖 20 節點矩陣是核心工具、再壓會傷可讀性）；若要更短須拆頁（移 ledger 或 map 到子頁＝IA 改動），本輪未做、未經站主拍板。

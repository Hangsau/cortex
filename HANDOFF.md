...
**Vortex 全 section 棄 techō、改套博物館說明牌（Museum Label）設計語言（2026-06-19，commit 00b1123，已 push + CI）**：站主要求用既有 web-ux-kit 研究改造 vortex 外觀，**風格不限但禁止沿用現在的 techō 樣式**，自己規劃/派工/驗收，並加一輪內容邏輯稽核（自打嘴巴），回來只看網站驗收。做法（只換視覺層，IA/JS/canonical/data 結構不動）：①風格選 DESIGN_SYSTEM 06 博物館說明牌 × web-ux-kit CJK 字體 token（決策見 `research/vortex-restyle-brief.md`），與 techō 暖手帳做最大反差。②**兩個 CSS 都換**：`vortex.css`（vx-* 子頁）+ `vortex-techo.css`（tx-* 首頁，**這檔最初漏改、首頁仍橘**，後補）——:root token 換展覽白牆 #F8F6F1 / 銅綠 #5B7B6F（大字·非文字）/ 深銅綠 #3E574D（小字 ≥4.5:1）/ museum 細線 #D8D4CC；首頁橫罫格線底紋移除；暖色 hover/fill（#fdfdfb/#f7efe6/#f4f1e8…）全 sed 換冷調 #FFFFFF/#EFEDE6；選取色換冷綠。③字體 13 個 layout 換 Cormorant Garamond + Noto Serif TC + Quattrocento Sans（棄 Shippori Mincho + Courier Prime）。④內文度量走 web-ux-kit CJK 處方（16px 下限·行高 1.8·行長 34rem）。**驗收**：本機 `hugo --baseURL localhost` build → python 靜態 server → headless Chrome 截圖 home/freestyle/water-sense-guide/levels/psychology/database/periodization/adm 八頁，全部 Museum Label 一致渲染、零 techō 殘留、master-detail/處境卡/短nav/L0–L6 脊椎 IA 保留。**踩雷**：①首頁用獨立 `vortex-techo.css`（tx-* class），改 vortex.css 改不到首頁，必須兩檔都改。②`hugo server --baseURL` 不覆寫 `.Site.BaseURL`（仍吐 production URL，截圖會載到線上舊 CSS）；要驗本機改動須用 `hugo --baseURL <local>` 完整 build 再靜態 serve。**內容稽核**（`research/vortex-content-logic-audit.md`，minimax 初稿 + Claude 抽查）：已 grep 驗證屬實的自打嘴巴 A-1（同檔 technical-analysis 行77「髖帶肩」vs 行192 Pink1991「肩胛主動發動」）、C-1（breast/fly 無 L0/L1）、C-2（udk/starts-turns 無 L-level）、C-3（首頁文案「四式共同地基」超 claim）。**泳式內容修正屬作者裁定 + canonical-first，本輪不擅改/不偽造，列清單交站主決定**（見報告 §0 Claude 驗證 + §7 優先序）。⚠ `.m3-*.txt` 與 `.prompts/audit-run.log` 刻意 untracked，勿 commit。

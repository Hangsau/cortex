...
## CSCS 題庫重寫：ch01–ch04 完成，配題表已上調 2.5 倍（2026-09-14）
...
19 道閘 0 錯）。ch05–ch24 尚未完成，照章號串行，**每章跑完兩輪 + 人工逐題查證 + commit
才派下一章**。
...
**接下來**：**下一章是 ch05**（`python tools/cscs_quiz_make_prompt.py ch05` 後發包，
發包一律帶 `--settings tools/cscs_quiz_delegate_settings.json`——那是 git 禁令閘）。
ch05 → ch24 走完（若 M3 在單次發包內寫不完 35 題，再考慮加 `--part K/N` 的分批機制；

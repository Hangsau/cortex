...
就攤不平；`topic_spread_note()` 是 prompt 端的唯一防線。ch24 生成後先看這一欄，
偏斜就直接 `--redo` 補偏少的 topic，不要留到人工查證階段。

ch24 照 ch23 同一套流程做，「`cscs_quiz_direct.py` 出題 → 過閘 → 跨題掃描 →
人工逐題查證 → commit」。
**⚠ ch23／ch24 的配題表是 `(40, 28, 12, 0, 13)`**——40 題、recall 28、application 12、
**analysis 0**、英文 13，和 ch04–ch22 每一章都不同形（ch22 仍是 `(38, 3, 18, 17, 13)`）。
recall 佔七成表示干擾項幾乎全要靠同章事實對照，而 G15 限定語閘只作用在
application／analysis，那兩章基本不會被它擋——**題幹品質要靠人工讀，不能指望閘**。
